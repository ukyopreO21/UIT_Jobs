...
func (r *JobRepository) FindByID(id int64) (*model.Job, error) {
...
func (r *JobRepository) FindByFields(fields map[string]string, searchValue string, page int, resultPerPage int) (data []any, pagination map[string]int, positions []string, faculties []map[string]any, err error) {
	...
	rows, err := r.DB.Query(query, valuesWithLimit...)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var job model.Job
		if err := rows.Scan(
			&job.Id, &job.Position, &job.Faculty, &job.Discipline, &job.Degree, &job.Deadline,
		); err != nil {
			return nil, nil, nil, nil, err
		}
